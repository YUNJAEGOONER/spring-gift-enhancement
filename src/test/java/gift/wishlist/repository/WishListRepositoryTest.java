...
class WishListRepositoryTest {
    ...
    void 위시리스트_생성(){
        ...
        ProductOption productOption = optionRepository.save(new ProductOption("실버그레이", 999, 15000, product));
        WishList wishList = wishListRepository.save(new WishList(member, productOption, 2));
    ...
    void 위시리스트_조회() {
        ...
        ProductOption productOption = optionRepository.save(new ProductOption("실버그레이", 999, 15000, product1));
        wishListRepository.save(new WishList(member, productOption, 5));

        Product product2 = productRepository.save(new Product("LG Gram", 1200000, "image"));
        ProductOption productOption2 = optionRepository.save(new ProductOption("화이트", 999, 15000, product2));
        wishListRepository.save(new WishList(member, productOption2, 3));
    ...
    void 특정_상품이_위시리스트에_존재하는지_확인() {
        ...
        WishList wishList = wishListRepository.save(new WishList(member, productOption, 5));
        Optional<WishList> wishListOptional = wishListRepository.findWishListByMemberIdAndProductOptionId(member.getMemberId(), productOption.getId());
